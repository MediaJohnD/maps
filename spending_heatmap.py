...
def main():
    ...
    parser.add_argument(
        "spend_csv", help="CSV file with ZIP or DMA and spend data"
    )
    parser.add_argument(
        "shapefile", help="Shapefile with ZIP or DMA boundaries"
    )
    ...
    parser.add_argument(
        "--spend-field", default="spend", help="Column with spend values"
    )
    parser.add_argument(
        "--output", default="heatmap.png", help="Output image file"
    )
    parser.add_argument(
        "--states", help="Optional shapefile with state boundaries"
    )
    ...
    if data_key is None:
        if "ZIP" in data.columns:
            ...
            raise ValueError(
                "Specify --data-key; could not infer column name."
            )
    ...
    merged = geo.merge(
        data[[data_key, args.spend_field]],
        left_on=geo_key,
        right_on=data_key,
        how="left",
    )
